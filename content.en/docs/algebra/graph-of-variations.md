...
\[
\frac{200}{1} = \frac{400}{2} = \frac{600}{3} = \frac{800}{4} = \frac{1000}{5}
\]
...
If we denote the variable weight as \( x \) and the variable cost as \( y \) we can express this algebraically as

\[
y = 200x
\]

The multiplying constant here is 200.
...
To identify direct variation is to look at the equation and determine if it is of the form

\[
y = kx
\]

where \( k \) is the constant of proportionality. Thus, an equation like \( y = 5x \) will always indicate direct proportion among variables.
...
Given the table of values, draw the graph and hence find

(i) the constant of variation

(ii) the circumference when diameter is 6 cm.

| Diameter (in cm) | 1 | 2 | 3 | 4 | 5 |
|---|---|---|---|---|---|
| Circumference (in cm) | 3.1 | 6.2 | 9.3 | 12.4 | 15.5 |
...
(ii) From the graph, \( y = \frac{5x}{6} \), if \( x = 90 \), then

\[
y = \frac{5}{6} \times 90 = 75 \text{ km}
\]

The distance travelled for 90 minutes is \( 75 \) km.

(iii) From the graph, \( y = \frac{5x}{6} \), if \( y = 300 \) then

\[
x = \frac{6y}{5} = \frac{6}{5} \times 300 = 360 \text{ minutes (or) 6 hours}
\]
...
The distance between Chennai and Madurai is (nearly) \( 480 \) km. Think of a train that starts from Chennai and travels towards Madurai. As it increases speed more and more, the time taken for travel will decrease. In the following table speed \( v \) is given in km/hr and time \( t \) is given in hours:

| Speed \( v \) (km/hr) | 30 | 40 | 60 | 80 |
|---|---|---|---|---|
| Time \( t \) (hours) | 16 | 12 | 8 | 6 |

From the table it is clear that if you travel at a slower speed, the time increases and if the train is faster, the time decreases. You find,

\[
30 \times 16 = 40 \times 12 = 60 \times 8 = 80 \times 6
\]

which tells that \( vt \) is a constant. Here, \( vt = 480 \). In such a case, we say the variables \( v \) and \( t \) are inversely proportional.
...
\[
y = \frac{k}{x} \Rightarrow xy = k, \quad k > 0
\]
...
\[
k = 40 \times 150 = 50 \times 120 = \ldots = 75 \times 80 = 6000
\]
...
Plot the points (40, 150), (50, 120), (60, 100) and (75, 80) and join to get a free hand smooth curve (Rectangular Hyperbola).
...
Also, from \( xy = 6000 \), if \( x = 120 \), then
...
Also, from \( xy = 6000 \), if \( y = 200 \), then
...
Draw the speed-time graph and use it to find the time taken by Kaushik with his speed of \( 2.4 \) km/hr.
...
\[
k = 12 \times 1 = 6 \times 2 = \ldots = 2 \times 6 = 12
\]
...
Already we learned that, the linear equation of straight line is

\[
y = mx + c
\]

where \( m \) is the slope of the straight line and \( c \) is the \( y \)-intercept. Also, the equation reduces to \( y = mx \) when the straight line passes through origin. As the graph of direct variation refer to straight line and its general form is \( y = kx \), we can conclude that 'constant of proportionality' is nothing but 'slope' of its straight line.
