...
From Fig.1.38, \( A = \{a,b,c,d\} \), \( B = \{1,2,3\} \) and \( f = \{(a,3), (b,3), (c,3), (d,3)\} \)

\( f(x) = 3 \ \forall x \in A \), Range of \( f = \{3\} \), \( f \) is a constant function.
...
\( f(x) = 3x - 5 \) can be written as \( f = \{(x, 3x - 5) \mid x \in \mathbb{R}\} \)
...
The function \( f \) is defined by three values in intervals I, II, III as shown by the side Fig. 1.40. For a given value of \( x = a \), find out the interval at which the point \( a \) is located, there after find \( f(a) \) using the particular value defined in that interval.
...
\[
\therefore f(x) = 3x - 2; \quad f(4) = 3(4) - 2 = 10
\]

(ii) \( x = -2 \) lies in the second interval.

\[
\therefore f(x) = x^2 - 2; \quad f(-2) = (-2)^2 - 2 = 4 - 2 = 2
\]
...
(iv) We know that \( f(1) = -1 \) and \( f(4) = 10 \). For finding \( f(-3) \), we see that \( x = -3 \), lies in the first interval.
